...
program SurfAniso
        ...
        meanAbs=sum(abs(cbst(1:dall)))/dall
        write(6 ,'(a, f8.3, a, f8.3,a, f5.1, a)') '  mean data weight:',sum(datweight(1:dall))/dall, &
        ' |  abs data mean with weight:',meanAbs,'s  |  dt/t0:',  meandeltaT, ' %'
        write(66,'(a, f8.3, a, f8.3,a, f5.1, a)') '  mean data weight:',sum(datweight(1:dall))/dall, &
        ' |  abs data mean with weight:',meanAbs,'s  |  dt/t0:',  meandeltaT, ' %'
        if (iso_mod) then
            norm=0
            do i=1,nar
        ...
        if (iso_inv)then
            write(*  ,'(a,2f8.2)') '  weight and  damp for dVs:',weightVs, damp
            write(66 ,'(a,2f8.2)') '  weight and  damp for dVs:',weightVs, damp
        else
            write(*  ,'(a,2f8.1)') '  weight and damp for Gcs: ',weightGcs, damp
            write(66 ,'(a,2f8.1)') '  weight and damp for Gcs: ',weightGcs, damp
        endif
        ...
        resNSigma2Norm=0
        if (iso_inv) then
            call CalVsReslNorm(maxvp,dall,GVs,dv, datweight, Tdata, fwdTvs, resbst, res2Norm, resNSigma2Norm)
        else
            fwdTaa=0
            call  CalGcsReslNorm(maxvp,dall,GGc,GGs,dv, datweight, Tdata, fwdTaa, resbst, res2Norm, resNSigma2Norm)
            ! update T_AA.
            RefTaa=0
            forall(i=1:dall)
                RefTaa(i)=fwdTaa(i)
        ...
        write(6 ,'(a,f10.3,a,f10.2,a,f10.2,a)') '  After Inversion: abs mean, std_devs and RMS of Res: ', &
        meanAbs,'s ', std_devs,'s ', dnrm2(dall,resbst,1)/sqrt(real(dall)),'s'
        write(66,'(a,f10.3,a,f10.2,a,f10.2,a)') '  After Inversion: mean, std_devs and RMS of Res: ', &
        meanAbs,'s ', std_devs,'s ', dnrm2(dall,resbst,1)/sqrt(real(dall)),'s'
