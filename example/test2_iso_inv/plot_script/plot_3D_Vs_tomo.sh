...
gmt set MAP_TICK_LENGTH_PRIMARY 4p/1p  # tick length
...
for ((i=1; i<=$Tnum; i=i+1)); do
    ...
    gmt makecpt -C$tomo_cpt_in -T$R_T -Z -I  > $tomo_cpt
    gmt grdimage tomo_grd -J$J -R$R_tomo -C$tomo_cpt  -Bxa1f1 -Bya1f1 -BWeSn+t"$title" ${offset[$i]} -K  -O >> $pic_name
    if  [ $i = 2  ]  ;then
    gmt psscale -D3.2c/-1.6c/7c/0.4ch -C$tomo_cpt -B$psscale_B -B"$psscale_By" -E  -O  -K  >> $pic_name
    fi
    rm tomo_grd
done
